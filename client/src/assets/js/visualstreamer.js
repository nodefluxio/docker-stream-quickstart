...
export function stopVisualisation() {
  Globalcontroller.map((data) => {
    data.controller.abort();
  });
}

function requestVisualisation(id, url, controller, ctx, loading) {
  loading.start();
  fetch(url, {
    ...
    .catch(() => {
      setTimeout(function() {
        requestVisualisation(id, url, controller, ctx, loading);
      }, 1500);
    });
}
...
function handleVisibilityChange() {
  if (document[hidden]) {
    ...
    Globalcontroller.map(data => {
      ...
      requestVisualisation(data.id, data.url, controller, data.ctx, data.loading);
    });
  }
}
...
function renderVisualisation(components) {
  for (var i = 0; i < components.length; i+=1) {
    ...
    var styleElem = container.appendChild(document.createElement("style"));
    styleElem.innerHTML =
    ...
    const url = components[i].dataset.url;
    const controller = new AbortController();
    ...
    if (url !== "") {
      requestVisualisation(i, url, controller, ctx, loading);
    }
  }
  
}

export async function showVisualisation(reload = false) {
  var divs = await document.getElementsByClassName("nodeflux-streamer");
  if (
